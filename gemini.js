...
export async function summarizeMessages(apiKey, model = MODEL, messages) {
  console.log('[Gemini] Starting message summarization', { messageCount: messages?.length });
...
function parseSummary(text) {
  ...
  if (!bulletsText) {
    const listItems = text.match(/^[-•*]\s*(.+)$/gmi) || [];
  ...
  console.debug('[Gemini] Parsed summary:', { tldrLength: tldr?.length, bulletCount: bullets?.length });
  return { tldr, bullets };
}
